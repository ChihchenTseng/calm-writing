...
採**壓力閾值觸發 ＋ 20 秒冷卻**機制（非速度判斷）：
- 真實觸控筆壓力 ≥ 45% 時觸發呼吸引導，觸發後以 `breathingTriggered` 旗標鎖定避免重複觸發
- 解除鎖定條件（兩者擇一即可，互不抵觸）：壓力降至 <35%，或呼吸引導結束後 20 秒冷卻期（`breathingCooldownUntil`）屆滿
- 維持壓力判斷而非改為速度判斷，理由：現有研究資料顯示 `avg_pressure` 與 VAS 壓力呈正相關趨勢，尚無證據顯示壓力判斷失準
- 速度雙層判斷（3 秒滑動窗平均速度 ≥ 180 pts/sec）僅為未來備案構想，**非現行實作**，若後續資料顯示壓力判斷失準才評估導入
- 觸發閾值（45%／35%）與冷卻時間（20 秒）調整，須比照 StressEngine 規則，需 PM 明確指示與資料依據
